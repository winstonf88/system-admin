...
} from "./components/CategoriesManager";
...
export default async function CategoriesPage() {
  const categoriesTreeRes = await fetchBackendAuthenticated(
    "/api/categories/tree",
  );
      ...
      <>
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-900/40 dark:bg-red-950/30">
          <p className="text-red-800 dark:text-red-200">
            Não foi possível carregar a árvore de categorias (HTTP{" "}
            {categoriesTreeRes.status}).
          </p>
        </div>
      </>
    );
