...
class UsersService:
    ...
    async def get_user_or_404(self, user_id: int) -> User:
        user = await User.get_or_none(id=user_id, tenant_id=self.tenant_context.tenant_id)
        if user is None:
            raise HTTPException(
    ...
    async def ensure_email_available(self, email: str, exclude_user_id: int | None = None) -> None:
